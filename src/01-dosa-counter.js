...
export function calculateDosaOrder(type, quantity = 1, isSpicy = false) {
    ...
    if (typeof type !== "string" || type.trim() === "" || !Object.keys(dosaTypes).includes(type))
        return null;

    if (isNaN(quantity) || quantity <= 0) return null;

    let pricePerDosa = isSpicy === true ? dosaTypes[type] + 10 : dosaTypes[type];

    let total = dosaTypes[type] * quantity;
    total = isSpicy === true ? (total += quantity * 10) : total;
